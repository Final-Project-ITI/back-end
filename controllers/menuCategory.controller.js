...
class MenuCategoryController {
  ...
  async getRestaurantMenuCategories(restaurantId) {
    return await this.categoryRepository.getRestaurantMenuCategories(
      restaurantId
    );
  }

  async getRestaurantMenuCategoryById(restaurantId, menuCategoryId) {
    return await this.categoryRepository.getRestaurantMenuCategoryById(
      restaurantId,
      menuCategoryId
    );
  }
  ...
  async createRestaurantMenuCategory(restaurantId, name, icon) {
    ...
    const categories =
      await this.categoryRepository.getRestaurantMenuCategories(restaurantId);
    ...
    const newCategory =
      await this.categoryRepository.createRestaurantMenuCategory({
        restaurantId,
        name,
  ...
  async updateRestaurantMenuCategoryById(
    restaurantId,
    menuCategoryId,
    val,
    icon
    ...
    const category =
      await this.categoryRepository.getRestaurantMenuCategoryById(
        restaurantId,
        menuCategoryId
      );
    ...
    if (!category) {
      ...
      category.restaurantId._id.toString() !== restaurantId.toString()
    ) {
  ...
  async deleteRestaurantMenuCategory(restaurantId, menuCategoryId) {
    ...
    return await this.categoryRepository.deleteRestaurantMenuCategory(
      restaurantId,
      menuCategoryId
    );
  }
